...
class MusicCore(commands.Cog, MusicPlayerMixin, MusicRadioMixin):
    ...
    @commands.command(name="status", aliases=["health", "diag", "diagnostico", "debug"])
    async def status_cmd(self, ctx):
        """Muestra el panel de diagnóstico técnico del sistema (exclusivo vía DM para usuarios Trusted)."""
        if ctx.guild is not None:
            await ctx.send("⚠️ El comando `status` solo está disponible en Mensajes Directos (DM) para usuarios de confianza.")
            return

        from database import is_user_trusted
        is_trusted = is_user_trusted(str(ctx.author.id))
        is_owner = await self.bot.is_owner(ctx.author)

        if not is_trusted and not is_owner:
            await ctx.send("❌ No tienes permisos de usuario Trusted para ver el diagnóstico del sistema.")
            return
        ...
        try:
            ...
            recsys_str = f"⚠️ Error consultando RecSys: {e}"

        # 4. Sistema y Caché
        try:
            temp_dir = tempfile.gettempdir()
            cache_files = [f for f in os.listdir(temp_dir) if f.startswith("cache_") or f.endswith(".webm") or f.endswith(".tmp")]
            cache_size_bytes = sum(os.path.getsize(os.path.join(temp_dir, f)) for f in cache_files if os.path.isfile(os.path.join(temp_dir, f)))
            active_vcs = sum(1 for p in self.players.values() if p.voice_client and p.voice_client.is_connected())

            sys_str = (
                f"🔊 **VCs Conectados**: `{active_vcs}` de `{len(self.players)}` servidores\n"
        ...
        embed.set_footer(text=f"Sesión Privada Trusted  •  @{ctx.author.name}")
